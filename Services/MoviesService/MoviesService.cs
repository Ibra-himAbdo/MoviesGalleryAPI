...
public class MoviesService : IMoviesService
{
    ...
    public MoviesService(DataContext context, IMapper mapper, IWebHostEnvironment hosting)
    {
        _context = context;
        _mapper = mapper;
        _hosting = hosting;
    ...
    public async Task<ServiceResponse<List<GetMovieDto>>> AddMovie(AddMovieDto newMovie)
    {
        ...
        if (newMovie.PosterFile is not null)
            movie.PosterURL = await SaveImage(newMovie.PosterFile);
    ...
    public async Task<ServiceResponse<GetMovieDto>> UpdateMovie(UpdateMovieDto updatedMovie)
    {
        ...
        try
        {
            var movie = await _context.Movies.FirstOrDefaultAsync(movie => movie.Id == updatedMovie.Id);
            ...
            {
                File.Delete(movie.PosterURL!);
                movie.PosterURL = await SaveImage(updatedMovie.PosterFile);
            }
    ...
    private async Task<bool> MovieExists(string title)
        => await _context.Movies.AnyAsync(movie => movie.Title == title);

    private async Task<string> SaveImage(IFormFile posterFile)
    {
        string imageFolderPath = Path.Combine(_hosting.WebRootPath, "posters");
        string posterPath = Path.Combine(imageFolderPath, posterFile.FileName);
